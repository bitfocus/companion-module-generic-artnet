...
class instance extends instance_skel {
	...
	init_actions() {
		this.setActions({
			...
			set_customvariable: {
				label: 'Set Value by Custom Variable',
				options: [
					{
						type: 'textinput',
						label: `Channel (Range 1-${MAX_CHANNEL})`,
				...
				callback: (action) => {
					this.parseVariables(action.options.channel, function (value) {
						action.options.channel = value;
					});
					let channel = Number(action.options.channel);

					this.parseVariables(action.options.value, function (value) {
						action.options.value = value;
					});
					let val = Number(action.options.value);

					this.parseVariables(action.options.duration, function (value) {
						action.options.duration = value;
					});
					let duration = Number(action.options.duration);

					this.transitions.run(channel - 1, isNaN(val) ? 0 : val, isNaN(duration) ? 0 : duration)
				},
			},
			set_complex: {
				label: 'Set Complex Value',
				options: [
					{
						type: 'textinput',
						label: `Channel (Range 1-${MAX_CHANNEL}), Value (Range 0-${MAX_VALUE});`,
						tooltip: 'Each parameter should be separated by a colon (,) and each group of parameters should be separated by a semicolon (;)',
						id: 'params',
						default: '1,0;'
					},
					{
				...
				callback: (action) => {
					...
					this.parseVariables(params, function (value) {
						params = value;
					});
					...
					for (let i = 0; i < params_groups.length; i++) {
						if (params_groups[i].indexOf(',') === -1) {
							this.log('error', 'One or more of your parameters are set incorrectly in the "Set Complex Value" action: ' + params_groups[i]);
							return;				
						}
						...
						let duration = action.options.duration;
						this.parseVariables(duration, function (value) {
							duration = value;
						});
			...
			offset_customvariable: {
				label: 'Offset Value by Custom Variable',
				options: [
					{
						type: 'textinput',
						label: `Channel (Range 1-${MAX_CHANNEL})`,
				...
				callback: (action) => {
					...
					let newval = Math.min(MAX_VALUE, Math.max(0, this.data[channel] + val)) // clamp to range

					this.transitions.run(channel - 1, isNaN(newval) ? 0 : newval, isNaN(duration) ? 0 : duration)
				},
			},
		})
	}
